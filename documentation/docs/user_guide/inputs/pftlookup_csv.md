...
| Fracherb       | `xfherbivore(nv)`                             | Fraction of net foliar production consumed by herbivores. Fraction of the turnover rate that affects senescence: <ul><li>if`Fracherb=1`: senescence is only related to cold and dry stress and there is no turnover factor</li><li> if `Fracherb=0` senescence has an additional turnover rate as set by leaf age. </li></ul>Used to calculate `casabiome%plantrate(nv,leaf)` used in calculation of plant pool senescence rate. Possibly herbivores were removed from CASA when coupled to CABLE. \( (-) \) |
