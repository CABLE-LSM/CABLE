...
!
!  2024-04-22 Edits for 27 tile configuration of CABLE
!  As of coding - we will assume the current 17 tile ordering applies for tiles 
!  1:13 of the 27 tile configuration, with the 4 non-veg tiles appended into 
!  tiles 24:27.  The additional tiles are filled with bare soil values.
!  
!  The current hard-wired ordering is retained in all other aspects.
!  
! ===============================================================================
...
CONTAINS
    SUBROUTINE tile_order(ntiles,t_sort)
        ...
        !set the tile order -----------------------------------------------------
        IF (ntiles == 27) THEN             !all tiles 27 tile version (CM3)
            t_sort = (/5,2,4,1,shrub,6,9,tundra,7,10,wetland17,6,6,              &
                           16,16,16,16,16,16,16,16,16,16,  16,14,15,17/)
        
        ELSEIF (ntiles == 17) THEN         !all tiles 17 tile version (CM2)         
            t_sort = (/5,2,4,1,shrub,6,9,tundra,7,10,wetland17,6,6,16,14,15,17/)
        
        ELSEIF (ntiles == 13) THEN         !veg tiles only (17 or 27 tiles)
    ...
    SUBROUTINE tile_resort_2D(ntiles, nvar, t_params)
        ...
        DO i = 1,ntiles
            ...
            !special cases
            IF ((ntiles==13) .or. (ntiles==17) .or. (ntiles==27)) THEN
