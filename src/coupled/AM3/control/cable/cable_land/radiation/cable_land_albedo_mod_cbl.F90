...
SUBROUTINE cable_land_albedo (                                                 &
  ...
  SoilTemp_CABLE, OneLyrSnowDensity_CABLE, SnowAge_CABLE, work, pars           &
)
!-------------------------------------------------------------------------------
! Description:
...
USE map_cable_parms_mod,       ONLY: map_cable_parms
...
LOGICAL, SAVE :: first_call = .TRUE.  !only initialize params on first call
...
IF( first_call) THEN 
  CALL map_cable_parms( mp, nsl, nrb, land_pts, nsurft, l_tile_pts,           &
                         ICE_SurfaceType,ICE_SoilType, dzsoil, work%veg,       &
                         work%soil, pars, tile_frac )
                                 
  ! Pack UM spatial (per landpoint) bare soil albedo to mp vector for CABLE
  CALL cable_pack_Albsoil( work%soil%albsoil, soil_alb, mp, nrb, l_tile_pts,               &
                           nsurft, land_pts )
...
CALL limit_HGT_LAI( work%veg%vlai, work%veg%hc, mp, land_pts, nsurft, npft,      &
                    surft_pts, surft_index, tile_frac, l_tile_pts,             &
                    LAI_pft_um, HGT_pft_um, CLAI_thresh )

! set Height of Canopy above snow (rough%hruff)
CALL HgtAboveSnow( HeightAboveSnow, mp, Cz0surf_min, work%veg%hc,              &
                   SnowDepth, SnowDensity )

! set Effective LAI considering potential snow coverage [cabopy%vlaiw]
CALL LAI_eff( mp, work%veg%vlai, work%veg%hc, HeightAboveSnow, reducedLAIdue2snow)

! Define logical mask for vegetated mp cell
CALL fveg_mask( veg_mask, mp, Clai_thresh, reducedLAIdue2snow )

work%reducedLAIdue2snow(1:mp) = reducedLAIdue2snow(:) 

!------------------------------------------------------------------------------
! 2. Call CABLE_rad_driver to run specific and necessary components of CABLE
!------------------------------------------------------------------------------
CALL cable_rad_driver( EffSurfRefl_beam, EffSurfRefl_dif, land_pts,            &
                       mp, nrb, ICE_SoilType, lakes_SurfaceType, Clai_thresh,  &
                       Ccoszen_tols, CGauss_w, Cpi, Cpi180, Cz0surf_min,       &
                       veg_mask, jls_standalone, jls_radiation, work%veg%iveg, &
                       work%soil%isoilm, work%veg%vlai, work%veg%hc,           &
                       SnowDepth, SnowDensity, SoilTemp, SnowAge,              &
                       work%soil%albsoil, coszen, work%veg%Xfang,              &
                       work%veg%Taul, work%veg%Refl, HeightAboveSnow,          &
                       reducedLAIdue2snow, ExtCoeff_beam, ExtCoeff_dif,        &
                       EffExtCoeff_beam,  EffExtCoeff_dif, CanopyTransmit_beam,&
                       CanopyTransmit_dif, CanopyRefl_beam,CanopyRefl_dif, c1, &
                       rhoch, xk, AlbSnow, RadFbeam, RadAlbedo, metDoY, SW_down)
...
CALL flush_local_vars( EffSurfRefl_beam, EffSurfRefl_dif,SnowDepth,            &
                       SnowDensity, SoilTemp, SnowAge, reducedLAIdue2snow,     &
