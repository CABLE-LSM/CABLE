   ...
   END IF
      ...
      IF( first_call ) THEN 
         ...
         IF (PrintStatus >= PrStatus_Diag) THEN
            WRITE(ummessage,*) 'maxval soil%drain_dens',maxval(soil%drain_dens,dim=1)
            CALL umPrint(umMessage,src='initialize_soil')

            WRITE(ummessage,*) 'minval soil%drain_dens',minval(soil%drain_dens,dim=1)
            CALL umPrint(umMessage,src='initialize_soil')
         END IF
