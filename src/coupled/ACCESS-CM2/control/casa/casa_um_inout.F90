...
!print *,'Les um_inout',mp,um1%land_pts,um1%ntiles
    ...
    !CM2!CALL casa_readphen(veg,casamet,phen)
...
!  WRITE(*,91) nyear,cplantsum,clittersum,csoilsum
  ...
  DO npt =1, mp
    ...
    ELSE
      casabal%sumnbal(npt)=MIN(9999.0,MAX(-9999.0,casabal%sumnbal(npt)))
    ENDIF
    ...
    ELSE
      casabal%sumpbal(npt)=MIN(9999.0,MAX(-9999.0,casabal%sumpbal(npt)))
    ENDIF
  ENDDO
