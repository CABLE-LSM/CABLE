...
! USE cable_surface_types_mod, ONLY : lakes_cable  !   should include this line to replace 16 everywhere below with lakes_cable - rk4417
...
! replaced above block by below - rk4417 - phase2
   ...
   litter_dz(:) = 0.0  
   IF (cable_user%litter) THEN
      WHERE (ssnow%isflag == 0 .OR. ssnow%snowd <= 0.1)
         litter_dz(:) = veg%clitt*0.003
      END WHERE
   END IF
...
! 2 statements below inserted by rk4417 - phase2
    ...
    DO i=1,mp
       IF (veg%iveg(i) .LT. 16) THEN   ! should probably replace 16 with lakes_cable - rk4417
          ...
          ! ccc Should include den_rat before ssnow%wbice for gw_model.
...
! ccc Possible other formulation? Found in MMY groundwater code branch.
