...
contains
  ...
  function cable_diagnostics_casa(casaflux, casapool, casamet) result(casa_output_variables)
    ...
    ! TODO(Sean): Currently the accumulation frequency for the following CASA
    ! outputs is set to time step accumulation (default) to restore bitwise
    ! reproducibility with the previous output module. These variables should
    ! ideally be accumulated at daily frequency to match the frequency at which
    ! CASA is executed in CABLE.
    ...
    casa_output_variables = [ &
      ...
      cable_output_variable_t( &
        ...
        metadata=[ &
          attribute("units", "umol/m^2/s"), &
          attribute("long_name", "Net Biosphere Production (uptake +ve)") &
        ] &
      ), &
      cable_output_variable_t( &
        field_name="dCdt", &
