...
module cable_netcdf_decomp_util_mod
  ...
  type dim_spec_t
    !* This type describes a single dimension in the shape specification of an
    ! array.
    ! The `name` component is used by the decomposition generating functions to
    ! identify dimensions which may be mapped from the in-memory layout of the
    ! array to the netCDF variable layout. The dimension(s) being mapped depend on
    ! the decomposition generating function being used.
...
contains
  ...
  function io_decomp_land_to_x_y(land_x, land_y, mem_shape_spec, var_shape_spec, type) result(decomp)
    ...
    type(dim_spec_t), intent(in) :: mem_shape_spec(:)
      !* A shape specification describing the shape and dimension names of the
