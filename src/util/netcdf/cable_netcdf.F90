...
module cable_netcdf_mod
  ...
  abstract interface
    ...
    function cable_netcdf_io_create_decomp(this, compmap, dims, type) result(decomp)
      ...
      integer, intent(in) :: compmap(:)
        !* An array of data offsets for each element of the array to be written.
        ! Each data offset describes where that element is located in the netCDF
        ! file.
  ...
  !> The I/O handler to use for all netCDF file operations. This is set by the
  ! module initialization procedure and should not be modified by user code.
