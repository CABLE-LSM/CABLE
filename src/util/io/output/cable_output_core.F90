...
! TODO(Sean): The preprocessor define ENFORCE_SINGLE_PRECISION_AGGREGATORS is
! enabled temporarily to restore bitwise reproducibility with the previous output
! module which enforces double precision variables to be sampled using single
! precision arrays, and enforces writing both double and single precision data as
! single precision.
#define ENFORCE_SINGLE_PRECISION_AGGREGATORS
...
#ifdef ENFORCE_SINGLE_PRECISION_AGGREGATORS
      if (restart_local) then
