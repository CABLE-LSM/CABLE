...
module cable_output_core_mod
  !* This module provides the core implementation of the CABLE output system.
  !
  ! This module is not intended to be used directly by other modules; use
  ! cable_output_mod instead.
  ...
  use cable_output_types_mod, only: allowed_reduction_methods
  use cable_output_types_mod, only: allowed_aggregation_methods
  ...
  use cable_output_reduction_buffers_mod, only: associate_temp_buffer
...
contains
  ...
  function cable_output_get_dimension(name) result(dim)
    ...
    select case(name)
    case ("patch")
      dim = cable_output_dim_t("patch", mp)
    case ("land")
      dim = cable_output_dim_t("land", mland)
    case ("land_global")
      dim = cable_output_dim_t("land_global", mland_global)
    case ("soil")
    ...
    case default
      call cable_abort("Unexpected dimension name " // name, __FILE__, __LINE__)
    end select
  ...
  subroutine cable_output_register_output_variables(output_variables)
    ...
    do i = 1, size(output_variables)
      associate(output_var => output_variables(i))
        ...
        if (allocated(output_var%data_shape)) then; if (.not. array_eq(output_var%data_shape(:)%size, output_var%aggregator%shape())) then
          call cable_abort("Data shape does not match aggregator shape for variable " // trim(output_var%field_name), __FILE__, __LINE__)
        end if; end if
  ...
  subroutine cable_output_write_parameters(time_index, patch, landpt)
  ...
  subroutine write_variable(output_profile, output_variable, patch, landpt, frame, restart)
    ...
    type is (aggregator_int32_1d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_int32_1d)
    ...
    type is (aggregator_int32_2d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_int32_2d)
        call first_patch_in_grid_cell( &
              input_array=aggregator%aggregated_data, &
    ...
    type is (aggregator_int32_3d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_int32_3d)
        call first_patch_in_grid_cell( &
              input_array=aggregator%aggregated_data, &
    ...
    type is (aggregator_real32_1d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_real32_1d)
    ...
    type is (aggregator_real32_2d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_real32_2d)
        call grid_cell_average( &
              input_array=aggregator%aggregated_data, &
    ...
    type is (aggregator_real32_3d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_real32_3d)
        call grid_cell_average( &
              input_array=aggregator%aggregated_data, &
    ...
    type is (aggregator_real64_0d_t)
      ...
      if (restart_local) then
        call output_profile%output_file%put_var(variable_name, real(aggregator%source_data, kind=SINGLE_PRECISION_ENFORCED_KIND))
        return
      end if
#else
    ...
    type is (aggregator_real64_1d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_real64_1d)
        call grid_cell_average( &
              input_array=aggregator%aggregated_data, &
    ...
    type is (aggregator_real64_2d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_real64_2d)
        call grid_cell_average( &
              input_array=aggregator%aggregated_data, &
    ...
    type is (aggregator_real64_3d_t)
      if (restart_local) then
        ...
        call associate_temp_buffer(output_variable, write_buffer_real64_3d)
        call grid_cell_average( &
              input_array=aggregator%aggregated_data, &
