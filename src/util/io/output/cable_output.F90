...
module cable_output_mod
  ...
  !
  ! The output system is responsible for writing CABLE output variables to disk
  ! in netCDF format, as well as writing CABLE restart files.
  !
  ! Using the output system involves the following steps:
  !
  ! 1. [[cable_output_mod_init]] must be called before any
  ! other procedures in this module to initialize the output system.
  ...
  ! `cable_output_variable_t` instances describing the output variables to be
  ! registered and passing this array to `cable_output_register_output_variables`.
  ! For example, a 1-dimensional diagnostic variable defined on the patch
  ...
  ! output variable is active, or if it is a restart variable. Please see
  ! [[cable_output_variable_t]] for more details. Output variables should be
  ! registered if their associated diagnostic working variables are initialised in
  ! the model as this can help provide information on the diagnostics which are
  ! available.
  ! 3. Output streams should be initialized via [[cable_output_init_streams]]. This
  ! should be done after registering output variables as the output stream
  ! initialization involves determining which output variables are active in each
  ! output stream based on the current output configuration. Now data can be
  ! written to disk via the output streams.
  ...
  ! a sampling frequency that matches the current time step.
  ! 6. If CABLE restart files are being written, then
  ...
  type, public :: cable_output_dim_t
    ...
    ! [[cable_output_get_dimension]] and is used to describe the dimensions of
    ! output variables in `cable_output_variable_t`.
    ! 
    ...
    ! reserved for special handling by the output module. For more details on how
    ! netCDF variable dimensions are inferred from `cable_output_dim_t` instances,
    ! please refer to [[native_to_netcdf_dimensions]].
    private
  ...
  type, public :: cable_output_variable_t
    ...
    character(64) :: accumulation_frequency = "all"
      !* The frequency at which the variable is accumulated. Please refer to
      ! the [[cable_timing_frequency_matches]] procedure for more information on the
      ! available frequency settings. If not specified, this defaults to "all",
      ! meaning that the variable is accumulated at every time step.
    ...
    character(64) :: aggregation_method = "point"
      ...
      ! aggregation methods. If not specified, this defaults to "point".
    logical :: active = .true.
    ...
    real, dimension(2) :: range = [-huge(0.0), huge(0.0)]
      !* The valid range of physical values for the native diagnostic (not to be
      ! confused with the netCDF variable which may have different units). If
      ! not specified, all values are considered valid.
    real :: scale_by = 1.0
  ...
  interface cable_output_mod_init
    module subroutine cable_output_implementation_init()
      !* Module initialization procedure for `cable_output_mod`.
      !
...
contains
  ...
  function cable_output_get_dimension(name) result(dim)
    ...
    ! dimension, add a new decomposition definition in `cable_output_decomp_mod`
