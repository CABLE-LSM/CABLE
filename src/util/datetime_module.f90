module datetime_module
  ...
  real(real64), parameter :: h2m = 60._real64 ! hour -> minute
  ...
  private :: calendarType, gregorian, julian, noLeaps, three60day
  enum, bind(c)
...
contains
  ...
  subroutine set_calendar(calendarString)
    ...
    if (trim(calendarString) == "gregorian") then
      calendar = gregorian
    elseif (trim(calendarString) == "julian") then
      calendar = julian
    elseif (trim(calendarString) == "noleaps") then
      calendar = noLeaps
    elseif (trim(calendarString) == "360day") then
      calendar = three60day
    else
      write(stderr,*) calendarString//" is not a valid calendar. "//&
  ...
  pure elemental integer function weekday(self)
    ...
    ! Assume other calendars return nonsense
    if (calendar == gregorian) then
      weekday = mod(self % day + ((month + 1) * 26) / 10 + k + k / 4 + j / 4 + 5 * j, 7) -1
    elseif (calendar == julian) then
      weekday = mod(self % day + ((month + 1) * 26) / 10 + k + k / 4 + 5 - j, 7) - 1
    end if
  ...
  pure elemental logical function isLeapYear(year)
    ...
    elseif (calendar == noLeaps .or. calendar == three60day) then
      isLeapYear = .false.
    end if
