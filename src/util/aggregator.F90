module aggregator_mod
  ...
  ! TODO(Sean): SINGLE_PRECISION_ENFORCED_KIND is used to restore bitwise
  ! reproducibility with the previous output module which accumulates double
  ! precision variables using single precision arrays. This should be removed once
  ! the new output module has been fully tested and validated against the old
  ! output module, and we are confident that the new output module is producing
  ! correct results without the need for single precision accumulators.
  integer, parameter, public :: SINGLE_PRECISION_ENFORCED_KIND = real32
  ...
  type, extends(aggregator_t) :: aggregator_real64_0d_t
    real(kind=SINGLE_PRECISION_ENFORCED_KIND), allocatable :: aggregated_data
    real(kind=real64), pointer :: source_data => null()
  end type aggregator_real64_0d_t

  type, extends(aggregator_t) :: aggregator_real64_1d_t
    real(kind=SINGLE_PRECISION_ENFORCED_KIND), dimension(:), allocatable :: aggregated_data
...
contains
  ...
  subroutine aggregator_init(this, method)
    ...
    type is (aggregator_real64_0d_t)
      if (.not. allocated(this%aggregated_data)) allocate( &
        this%aggregated_data, mold=real(this%source_data, kind=SINGLE_PRECISION_ENFORCED_KIND) &
      )
  ...
  subroutine mean_accumulate(this, scale, div, offset)
    ...
    type is (aggregator_real64_0d_t)
      this%aggregated_data = this%aggregated_data + ( &
        real( &
          scale_val * this%source_data / div + offset_val, kind=SINGLE_PRECISION_ENFORCED_KIND &
  ...
  subroutine sum_accumulate(this, scale, div, offset)
    ...
    type is (aggregator_real64_0d_t)
      this%aggregated_data = this%aggregated_data + real( &
        scale_val * this%source_data / div_val + offset_val, kind=SINGLE_PRECISION_ENFORCED_KIND &
      )
    type is (aggregator_real64_1d_t)
      this%aggregated_data = this%aggregated_data + real( &
        scale_val * this%source_data / div_val + offset_val, kind=SINGLE_PRECISION_ENFORCED_KIND &
  ...
  subroutine min_reset(this)
    ...
    type is (aggregator_real32_3d_t)
      this%aggregated_data = huge(real(0.0_real32))
    type is (aggregator_real64_0d_t)
      this%aggregated_data = huge(real(0.0_SINGLE_PRECISION_ENFORCED_KIND))
    type is (aggregator_real64_1d_t)
      this%aggregated_data = huge(real(0.0_SINGLE_PRECISION_ENFORCED_KIND))
    type is (aggregator_real64_2d_t)
      this%aggregated_data = huge(real(0.0_SINGLE_PRECISION_ENFORCED_KIND))
  ...
  subroutine other_reset(this)
    ...
    type is (aggregator_real32_3d_t)
      this%aggregated_data = 0.0_real32
    type is (aggregator_real64_0d_t)
      this%aggregated_data = 0.0_SINGLE_PRECISION_ENFORCED_KIND
    type is (aggregator_real64_1d_t)
      this%aggregated_data = 0.0_SINGLE_PRECISION_ENFORCED_KIND
    type is (aggregator_real64_2d_t)
      this%aggregated_data = 0.0_SINGLE_PRECISION_ENFORCED_KIND
