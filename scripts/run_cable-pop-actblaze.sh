...
#this is the section specifically for inh599 on gadi
sitepath="/scratch/x45/inh599/BIOStests/${experiment}" # Results
workpath="/home/599/inh599/JAC/CABLE/newBLAZE" # run directory
cablehome="/home/599/inh599/JAC/git/CABLE-POP-BLAZE/CABLE/"
# Cable executable
if [[ ${dompi} -eq 1 ]] ; then
    exe="${cablehome}/offline/cable-mpi"   #not valid
else
...
BlazeDataPath="/g/data/x45/Data_BLAZE"
# Global Mask
SurfaceFile="${aux}/offline/gridinfo_CSIRO_CRU05x05_4tiles.nc"
GlobalLandMaskFile="/g/data/x45/BIOS3_forcing/acttest9/acttest9" # no file extension
...
# Cable parameters - inh599 edits here
if [[ "${mettype}" == 'bios' ]] ; then
    namelistpath="${workpath}/namelists_bios"
    namelistcrupath="${workpath}/namelists_bios"
    filename_veg="${cablehome}/params/v11/def_veg_params.txt"
    filename_soil="${cablehome}/params/v11/def_soil_params.txt"
    casafile_cnpbiome="${cablehome}/params/v11/pftlookup.csv"
else
...
cat > ${tmp}/sedtmp.${pid} << EOF
    ...
    cable_user%c13o2_simple_disc       = .true.
