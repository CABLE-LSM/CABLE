...
!!$             nyear =INT((kend-kstart+1)/(365*ktauday))
  ...
  SUBROUTINE worker_cable_params (comm,met,air,ssnow,veg,bgc,soil,canopy,&
    ...
    ! plant hydraulics, mgdk576, 23/07/2019: +3 (r2) Matrix (s1)
    bidx = bidx + 1
...
!!$
    ...
    bidx = bidx + 1
    CALL MPI_Get_address (veg%P12, displs(bidx), ierr)
    blen(bidx) = r1len
    ...
    bidx = bidx + 1
    CALL MPI_Get_address (veg%P88, displs(bidx), ierr)
    blen(bidx) = r1len

    bidx = bidx + 1
    CALL MPI_Get_address (veg%b_plant, displs(bidx), ierr)
    blen(bidx) = r1len

    bidx = bidx + 1
    CALL MPI_Get_address (veg%c_plant, displs(bidx), ierr)
    blen(bidx) = r1len

    bidx = bidx + 1
    CALL MPI_Get_address (veg%kmax, displs(bidx), ierr)
    blen(bidx) = r1len

    bidx = bidx + 1
    CALL MPI_Get_address (veg%PLCcrit, displs(bidx), ierr)
    blen(bidx) = r1len
    ...
    bidx = bidx + 1
    CALL MPI_Get_address (canopy%psi_can, displs(bidx), ierr)
    blen(bidx) = r1len
    ...
    bidx = bidx + 1
    CALL MPI_Get_address (canopy%plc_sat, displs(bidx), ierr)
    blen(bidx) = r1len
    ...
    bidx = bidx + 1
    CALL MPI_Get_address (canopy%day_plc_sat, displs(bidx), ierr)
    blen(bidx) = r1len
  ...
  SUBROUTINE worker_outtype (comm,met,canopy,ssnow,rad,bal,air,soil,veg)
    ...
    !  plant hydraulics, mgdk576, 23/07/2019: +3 Matrix (s2)
    bidx = bidx + 1
    ...
    ! plant hydraulics; mgdk576 2019; ms8355
    bidx = bidx + 1
