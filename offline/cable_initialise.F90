...
CONTAINS
  ...
  SUBROUTINE get_restart_data(ssnow, canopy, bgc, bal, veg, rad, EMSOIL)
    ...
    CALL readpar(ncid_rin,'extkn',dummy,veg%extkn,filename%restart_in, &
         max_vegpatches,'def',from_restart,mp)
    CALL readpar(ncid_rin,'alpha',dummy,veg%alpha,filename%restart_in, &
         max_vegpatches,'def',from_restart,mp)
     CALL readpar(ncid_rin,'vcmax_scalar',dummy,veg%vcmax_scalar,filename%restart_in, &
         max_vegpatches,'def',from_restart,mp)
     CALL readpar(ncid_rin,'P50',dummy,veg%P50,filename%restart_in, &
         max_vegpatches,'def',from_restart,mp)
    ...
    CALL readpar(ncid_rin,'g3',dummy,veg%g3,filename%restart_in, &
     ...
     CALL readpar(ncid_rin,'kmax',dummy,veg%kmax,filename%restart_in, &
                max_vegpatches,'def',from_restart,mp) 
     CALL readpar(ncid_rin,'a1gs',dummy,veg%a1gs,filename%restart_in, &
                max_vegpatches,'def',from_restart,mp) 
     CALL readpar(ncid_rin,'d0gs',dummy,veg%d0gs,filename%restart_in, &
                max_vegpatches,'def',from_restart,mp) 
