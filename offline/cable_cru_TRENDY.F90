...
INTEGER, PRIVATE, PARAMETER  :: nSuppVariables = 4
...
TYPE CRU_TYPE
  ...
  TYPE(CRU_MET_TYPE), DIMENSION(nVariables + nSuppVariables) :: Met
...
SUBROUTINE BIOS_GET_SUBDIURNAL_MET(CRU, Met, CurYear, ktau)
  ...
  IF (NewDay) THEN
    ...
    WRITE(*,*) "CO2: ", Met%ca
    WRITE(*,*) "Rain: ", CRU%Met(rain)%MetVals
    WRITE(*,*) "Tmin: ", CRU%Met(Tmin)%MetVals
    WRITE(*,*) "Tmax: ", CRU%Met(Tmax)%MetVals
    WRITE(*,*) "vph0900: ", CRU%Met(vph0900)%MetVals
    WRITE(*,*) "vph1500: ", CRU%Met(vph0900)%MetVals
