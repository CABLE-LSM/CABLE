...
CONTAINS
    ...
    SUBROUTINE get_fluxes_and_derivs( &
        ...
        REAL(r_2),          DIMENSION(1:mp)                    :: hice
             ...
             if (vsnow(kk)%nsnow.ge.1) then
                ...
                keff = 2_r_2*(vsnow(kk)%kth(vsnow(kk)%nsnow)*var(1)%kth)/ &
    ...
    SUBROUTINE estimate_timestep( &
        ...
        REAL(r_2),          DIMENSION(1:n) :: iqex
             ...
             where (var(1:n)%iice==1)  tmp2d1(1:n) =  tmp2d1(1:n)/2.
             ...
             if (advection==1) then
                ...
                tmp2d2(1:n) = abs((qh(1:n)-qadv(1:n))-(qh(0:n-1)-qadv(0:n-1)))/(var(1:n)%csoileff*dx(1:n)) ! first order estimate of rate of temperature change
                deltaTmax(1:n) = tmp2d2(1:n)*(tfin-t(kk)) ! maximum  T change

!!$
!!$                if (irec.eq. 2004 .and. kk.eq.2) then
!!$                   write(*,*) 'guess 1', tmp2d2(1), deltaTmax(1)
!!$                endif
!!$                ! revised   rate of temperature change in topsoil layer
!!$                tmp2d2(1) =abs((qh(1)-qadv(1)+0.5*(qhTa(1)-qadvTa(1))*deltaTmax(1)+&
!!$                     0.5*(qhTb(1)-qadvTb(1))*deltaTmax(2)) - &
!!$                     (qh(0)+0.5*(qhTb(0)-qadvTb(0))*deltaTmax(1)-qadv(0)))/ &
!!$                     (var(1)%csoileff*dx(1)) 
!!$                tmp2d2(1) =abs((qh(1)-qadv(1)) - &
!!$                     (qh(0)+0.5*(qhTb(0)-qadvTb(0))*deltaTmax(1)-qadv(0)))/ &
!!$                     (var(1)%csoileff*dx(1)) 
!!$
!!$                tmp2d2(1) =abs((qh(1)-qadv(1)+0.5*(qhTa(1)-qadvTa(1))*deltaTmax(1)+&
...
!!$                     
!!$                     if (irec.eq. 2004 .and. kk.eq.2) then
!!$                write(*,*) 'guess 2', tmp2d2(1), qh(0) -qadv(0), 0.5*(qhTb(0)-qadvTb(0))*deltaTmax(1)
!!$             endif
...
!!$                    write(*,*) 'dphidT', var(1)%phiT, var(2)%phiT
!!$                    write(*,*) 'dphidS', var(1)%phiS, var(2)%phiS
...
!!$                  endif
        ...
        hsnow(:) = zero
          ...
          do while (again(kk)) ! sometimes need twice to adjust phi at satn
             ...
             if (.not. again(kk))  then
                ...
                !dwoff(kk) = min(dwoff(kk),max((q(0)-qprec_snow(kk))*dt(kk),zero))
            ...
            if (.not. again(kk)) then
                ...
                elseif ((var(i)%iice==1).and.((J0(i) + LHS_h(i)*dt(kk))<JSoilLayer(Tfreezing(kk), &
                    ...
                    if (Tsoil(i) .lt. -40.0) then
                        ...
                        tmp1 = GTfrozen(real(Tsoil(i)- dTsoil(i)-50., r_2), tmp1d2(kk), dx(i), theta,par(i)%css, par(i)%rho, &
    ...
    SUBROUTINE update_unknowns( &
        ...
        if (.not. again(kk)) then
            ...
            !                 (qhsig(-vsnow(kk)%nsnow)-qadvsig(-vsnow(kk)%nsnow))*dt(kk) - &
            !                 (qadvsig(0)*dt(kk) - qadvsig(n)*dt(kk))) .gt. 1) then
            !                 write(*,*) 'E balance check 2', irec, nsteps,  &
            !                 Jcol_latent_S(kk) + dJcol_latent_T(kk) + dJcol_sensible(kk) - &
            !                 (qhsig(-vsnow(kk)%nsnow)-qadvsig(-vsnow(kk)%nsnow))*dt(kk) - &
            !                 (qadvsig(0)*dt(kk) - qadvsig(n)*dt(kk))
            !                 stop
            !                 endif
    ...
    SUBROUTINE get_and_solve_eqn( &
        ...
        INTEGER(i_d)                                           :: advection, septs ! switches
        REAL(r_2)                                              :: c2, theta
        ...
        do while (iok(kk)==0) ! keep reducing time step until all ok
            ...
            if (septs == 1) then ! uncoupled of T and S
                ...
                ! if (any(abs(tmp2d1(1:n))>1.e-6)) then
                !!$                   if (any(abs(abs(LHS_h(1:n)/RHS_h(1:n))-one) > 1e-6)) then
                !!$                      write(*,*) "energy imbalance after matrix solution"
                !!$                      write(*,*) "LHS: ", LHS_h(1:n)
                !!$                      write(*,*) "RHS: ", RHS_h(1:n)
                !!$                      write(*,*) "LHS-RHS: ", LHS_h(1:n)-RHS_h(1:n)
                !!$                      write(*,*) "LHS/RHS-1: ", abs(LHS_h(1:n)/RHS_h(1:n))-one
                !!$                      write(*,*) "irec,kk, nsteps:" , irec,kk, nsteps(kk)
                !!$                      write(*,*) "S 1 ", S(1)
                !!$                      write(*,*) "ice 1: ", var(1)%thetai, var(1)%iice, var(1)%isat
                !!$                      write(*,*) "ns: ", ns(kk)
                !!$                      write(*,*) cch(1)*dy(1) +ddh(1)*de(1) + eeh(1)*dy(2) + ffh(1)*de(2)
                !!$                      write(*,*) ggh(1), -(qh(0) - qh(1))/sig(kk)
                !!$                      write(*,*) "dy, de", dy(1), de(1)
                !!$                      write(*,*) qhsig(1), qhsig(0),qh(1), qh(0)
                !!$                      write(*,*) G0(kk), qhya(0), qhyb(0), qhTa(0), qhTb(0)
                !!$                      write(*,*) qadvsig(1), qadvsig(0),qadv(1), qadv(0)
                !!$                      write(*,*) "qprec, qprec_snow", qprec(kk), qprec_snow(kk)
                !!$                      write(*,*) (dx(1)*var(1)%csoileff)*dTsoil(1)/dt(kk) - &
                !!$                           dy(1)*par(1)%thre*dx(1)*rhow*lambdaf/dt(kk)*var(1)%iice* &
                !!$                           real(1-var(1)%isat,r_2) + &
                !!$                           var(1)%dthetaldT*(Tsoil(1))*rhow*(cswat-csice)*dx(1)*dTsoil(1)/dt(kk)* &
                !!$                           var(1)%iice
                !!$                      write(*,*)  real(var(1:1)%iice,r_2) * real(1-var(1:1)%isat,r_2) * &
                !!$                           dx(1:1) * dy(1:1)/dt(kk) * par(1:1)%thre * rhow * csice * (Tsoil(1:1))
            ...
            if (.not. again(kk)) then
                ...
                if (vsnow(kk)%nsnow>0) then
                    do i=1, vsnow(kk)%nsnow
                        if ((vsnow(kk)%hsnow(i) + dy(i-vsnow(kk)%nsnow))<zero) then
                            fac(kk) = 0.99*vsnow(kk)%hsnow(i)/abs(dy(i-vsnow(kk)%nsnow))
                            nfac6(kk) = nfac6(kk)+1
                            iok(kk) = 0
                            exit
                        endif
                        ...
                        if (vsnow(kk)%hliq(i)>zero) then
                            ...
                            if ((vsnow(kk)%hliq(i) + de(i-vsnow(kk)%nsnow))<-0.01*vsnow(kk)%hsnow(i)) then
                                fac(kk) = (-0.009*vsnow(kk)%hsnow(i)-vsnow(kk)%hliq(i))/de(i-vsnow(kk)%nsnow)
                                nfac8(kk) = nfac8(kk)+1
                                iok(kk) =0
                                exit
                            endif
                        elseif ((vsnow(kk)%tsn(i) +  de(i-vsnow(kk)%nsnow)).gt.zero) then
                            ...
                            if (delta_snowliq(i).gt.(vsnow(kk)%hsnow(i) + dy(i-vsnow(kk)%nsnow))) then
                                fac(kk) = 0.9*vsnow(kk)%hsnow(i)/(delta_snowliq(i)- dy(i-vsnow(kk)%nsnow))
                ...
                ! if (litter .and. ns(kk)==1) then ! litter, no pond
                !    if (-dy(0) > SL(kk)*dSmaxr) then ! Check relative litter moisture change
                !       fac(kk) = min(fac(kk),max(half,accel(kk)*dSmaxr*SL(kk)/(-dSfac*dy(0))))
                !       iok(kk) = 0
                !    endif
                !
                !    if (abs(deltaTa(kk)) > zero) then
                !       tmp1d1(kk) = de(0) - deltaTa(kk)
                ...
                if (iok(kk)==0) then
                    ...
                    !h0_tmp(kk) = h0(kk) + (qsig(0)-qsig(1))*dt(kk)
...
!!$                 endif
           ...
           ! write(wlogn,"(3i8,1f8.2)") irec, kk, nsteps(kk), Tsoil(1)
  ...
  SUBROUTINE timestep_loop( &
          ...
          drainage, discharge, qh, nsteps, vmet, vlit, vsnow, var, T0, Tsurface, Hcum, lEcum, deltaice_cum_T, deltaice_cum_S, Gcum, &
          ...
          isotopologue, advection, septs , c2, theta, dTqwdTa, dTqwdTb, Tqw, keff, cp, cpeff, hice, deltahice, h0_0, hice_0, &
    ...
    deltacv_ss= zero
       ...
       do while (t(kk) < tfin)
          ...
          if (isotopologue /= 0) then
             ...
             thetasat_ss(itop:0) = 1.0
             thetar_ss(itop:0) = 1.0
             ...
             if (vsnow(kk)%nsnow.gt.0) then
                ...
                tmp_tortuosity(itop:0) = 0.8
                thetasat_ss(itop:0) = 1.0
                thetar_ss(itop:0) = 0.0
...
!    REAL(r_2),    DIMENSION(1:mp,1:n)   :: delthetai, dthetaldT, thetal
    ...
    !                 Jcol_latent_S, Jcol_latent_T, Jcol_sensible, deltaJ_sensible_S, qmelt, qtransfer, j0snow)
      J0snow(kk) = vsnow(kk)%J ! for tracking change in internal energy of snowpack
       ...
       CALL timestep_loop( &
           tfin, irec, mp, qprec, qprec_snow, n, dx(kk,:), h0, S(kk,:), thetai(kk,:), Jsensible(kk,:), Tsoil(kk,:), evap, runoff, &
           infil, drainage, discharge, qh, nsteps, vmet, vlit, vsnow, var(kk,:), T0, Tsurface, Hcum, lEcum, deltaice_cum_T, &
           deltaice_cum_S, Gcum, Qadvcum, Jcol_sensible, Jcol_latent_S, Jcol_latent_T, csoil(kk,:), kth(kk,:), phi(kk,:), dxL, zdelta, &
           SL, Tl, plit, par(kk,:), wex, ciso, cisoice, ciso_snow, cisoice_snow, cisos, cprec, cprec_snow, qali, qiso_in, qiso_out, &
           qiso_evap_cum, qiso_trans_cum, qiso_liq_adv, qiso_vap_adv, qiso_liq_diff, qiso_vap_diff, qvsig, qlsig, qvTsig, qvh, deltaTa, &
           ...
           cvL0, SLliq0, deltacvL, SLliq, deltaSLliq, qiso_evap, qiso_trans, lE0, G0, Epot, Tfreezing, dtdT, LHS(kk,:), RHS(kk,:), LHS_h(kk,:), RHS_h(kk,:), &
           surface_case, nns, iflux, litter, i, j, k, kk, condition, littercase, isotopologue, advection, septs , c2, theta, dTqwdTa, &
           dTqwdTb, Tqw, keff, cp, cpeff, hice, deltahice, h0_0, hice_0, h0_tmp, hice_tmp, qtransfer, qmelt_ss , qprec_ss, &
           cprec_ss, delta_snowcol(kk,:), delta_snowT(kk,:), delta_snowliq(kk,:), thetai_0(kk,:), J0(kk,:), tmp1, tmp2, iqex(kk,:), icali, nfac1, nfac2, &
  ...
  SUBROUTINE snow_adjust(irec, mp, n, kk, ns, h0, hice, thetai, dx, vsnow, var, par, S, Tsoil, &
    ...
    if (((vsnow(kk)%wcol-sum(vsnow(kk)%hliq(:)))<snmin*(vsnow(kk)%dens(1)/rhow)).or. &
          ...
          if ((tmp1d1(kk)+tmp1d2(kk)-tmp1d4(kk))>zero) then !  complete melting
             ...
             if ((tmp1*tmp2) < zero) then
                tmp1d3(kk) = rtbis_Tfrozen(tmp1d2(kk), dx(1), theta,par(1)%css, par(1)%rho, &
                     h0(kk), par(1)%thre, par(1)%the, par(1)%he, one/(par(1)%lambc*freezefac), &
                     real(Tsoil(1)-50., r_2), Tfreezing(kk), real(0.0001,r_2))
       ...
       if ((vsnow(kk)%hliq(1) - vsnow(kk)%hsnow(1)*vsnow(kk)%fsnowliq_max(1))>zero) then ! remove melt water
          ...
          qmelt(1) = min(qmelt(1), max(0.9*(vsnow(kk)%hsnow(1)-snmin*(vsnow(kk)%dens(1)/rhow)),zero))
          ...
          !vsnow(kk)%dens(1) = max(vsnow(kk)%dens(1) - rhow*qmelt(1)/vsnow(kk)%depth(1), 50.0)
          do i=1, vsnow(kk)%nsnow
             vsnow(kk)%Jsensible(i) = (vsnow(kk)%hsnow(i)-vsnow(kk)%hliq(i))*rhow*csice*(vsnow(kk)%Tsn(i))
             vsnow(kk)%Jlatent(i) = (vsnow(kk)%hsnow(i)-vsnow(kk)%hliq(i))*rhow*(-lambdaf)
          enddo
