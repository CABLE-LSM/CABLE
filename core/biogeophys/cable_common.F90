...
MODULE cable_common_module
  ...
  TYPE kbl_user_switches
     ...
     CHARACTER(LEN=10) :: &
          g1_Tcorr               = 'none'          ! g1 T correction: 'none', 'cheesman', 'lu'
     INTEGER ::  &
     ...
     character(len=20) :: wstress = 'none'  ! water stress variable: 'fwsoil' or 'fwpsi'
  ...
  TYPE vegin_type
     REAL, DIMENSION(:), ALLOCATABLE :: &
          ...
          psi_delta   ! prescribed margin: psi_wilt = psi_critical - psi_delta (MPa, positive)
...
CONTAINS
  ...
  SUBROUTINE get_type_parameters(logn, vegparmnew, classification)
    ...
    ALLOCATE (                                                            &
         ...
         vegin%psi_critical(mvtype), vegin%psi_delta(mvtype)   )
    ...
    IF( vegparmnew ) THEN    ! added to read new format (BP dec 2007)
       ...
       DO a = 1,mvtype
          ...
          READ(vegunit,*) vegin%a1gs(jveg), vegin%d0gs(jveg), vegin%alpha(jveg), &
               vegin%convex(jveg), vegin%cfrd(jveg)
          READ(vegunit,*) vegin%gswmin(jveg), vegin%conkc0(jveg), vegin%conko0(jveg), &
               vegin%ekc(jveg), vegin%eko(jveg)
          ! added g1tuzet in config file
          ...
          READ(vegunit,*) vegin%kmax(jveg), vegin%PLCcrit(jveg), vegin%psi_critical(jveg), &
               vegin%slope_soil(jveg), vegin%psi_delta(jveg)
...
!#endif
  ...
  SUBROUTINE init_veg_from_vegin(ifmp, fmp, veg)
    ...
    DO h = ifmp, fmp          ! over each patch in current grid
       ...
       veg%psi_delta(h)      = vegin%psi_delta(veg%iveg(h))
