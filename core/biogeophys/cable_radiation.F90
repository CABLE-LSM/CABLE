  ...
  SUBROUTINE radiation( ssnow, veg, air, met, rad, canopy )
    ...
    WHERE (canopy%vlaiw > C%LAI_THRESH )
       ...
       rad%gradis(:,1) = ( 4.0 * C%EMLEAF / (C%CAPP * air%rho) ) * flpwb        &
            / (met%tvrad) * rad%extkd                              &
!$            / (met%tk) * rad%extkd                              &    ! MMY has this line instead ? -- rk4417 ! MMY@23Apr2023 need to test whether tvrad and tk the same
...
!$            flpwb / met%tk * rad%extkd *                        &    ! MMY has this line instead ? -- rk4417 ! MMY@23Apr2023 need to test whether tvrad and tk the same
