      ...
      if (APAR(k) .gt. 60e-6) then
         ...
         if (cable_user%explicit_gm) then
            if (TRIM(cable_user%g0_switch) == 'default') then
               CALL fabcd(cs(k), g0(k)*fwsoil(k), x, gamma, beta, gammastar, Rd, gm, a, b, c1, d)
            ...
            elseif (TRIM(cable_user%g0_switch) == 'maximum') then
               ...
               if (g0(k)*fwsoil(k) .gt. Anc*x/cs(k)) then
                  CALL fabcd(cs(k), g0(k)*fwsoil(k), real(0.0), gamma, beta, gammastar, Rd, gm, a, b, c1, d)
                  CALL fpq(a,b,c1,d,p,q)
                  CALL fAm(a,b,c1,d,p,q,Anc)
               endif
            endif   
            ...
            if (TRIM(cable_user%g0_switch) == 'default') then
               CALL fabc(cs(k), g0(k)*fwsoil(k), x, gamma, beta, gammastar, Rd, a, b, c1)
               CALL fAn(a,b,c1,Anc) ! rubisco-limited
            elseif (TRIM(cable_user%g0_switch) == 'maximum') then
               CALL fabc(cs(k), real(0.0), x, gamma, beta, gammastar, Rd, a, b, c1)
               CALL fAn(a,b,c1,Anc) ! rubisco-limited
               if (g0(k)*fwsoil(k) .gt. Anc*x/cs(k)) then
                  CALL fabc(cs(k), g0(k)*fwsoil(k), real(0.0), gamma, beta, gammastar, Rd, a, b, c1)
                  CALL fAn(a,b,c1,Anc) ! rubisco-limited
               endif
            endif
         endif
...
REAL FUNCTION total_photosynthesis(bjv) 
  ...
  INTEGER   :: k, j
  REAL      :: kct, kot, tdiff
  REAL      :: x, gamma,  beta, gammastar, Rd, jmaxt, gm, trf
