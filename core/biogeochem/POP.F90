...
MODULE POP_Types
  ...
  TYPE Patch
     TYPE(layer), DIMENSION(:), ALLOCATABLE :: Layer
...
CONTAINS
    ...
    DO g=a, b
      ...
      DO k=1, NPATCH2D
        ...
        DO L = 1, NLayer
          ...
          END DO ! Finish iterating over cohorts
        END DO ! Finish iterating over layers
    ...
    IterateCells: DO g = 1, np
      IteratePatches: DO P = 1, NPATCH2D
        IF (POP%POP_grid(g)%patch(P)%age(1) > it(g)) THEN
          it(g) = POP%POP_grid(g)%patch(P)%age(1)
        END IF
       ...
       DO k=1,NPATCH2D
          ...
          IF (pop%pop_grid(j)%patch(k)%first_disturbance_year(idisturb).NE.0) THEN
             IF ((pop%pop_grid(j)%patch(k)%first_disturbance_year(idisturb).EQ.pop%pop_grid(j)%patch(k)%age(idisturb)).or. &
                ...
                pop%pop_grid(j)%patch(k)%layer(1)%ncohort = 0
                pop%pop_grid(j)%patch(k)%layer(1)%biomass = 0.0_dp
                pop%pop_grid(j)%patch(k)%layer(1)%density = 0.0_dp
                pop%pop_grid(j)%patch(k)%layer(1)%hmean = 0.0_dp
                pop%pop_grid(j)%patch(k)%layer(1)%hmax  = 0.0_dp
                pop%pop_grid(j)%patch(k)%age(idisturb) = 0
                pop%pop_grid(j)%patch(k)%first_disturbance_year(idisturb) = 0
  ...
  SUBROUTINE INTERPOLATE_BIOMASS_1D(pop, disturbance_interval,it,g)
    ...
    ! Patch iterator
    INTEGER(i4b), DIMENSION(:), ALLOCATABLE :: PatchAges
    INTEGER :: P
    REAL(dp) :: NPoints
    ...
    ! To more closely match the original code, pull out the patch ages into an
    ! intermediate array to operate on
    ALLOCATE(PatchAges(NPATCH2D))
    ConstructAgeArray: DO P = 1, NPATCH2D
      PatchAges(P) = POP%pop_grid(g)%patch(P)%age(1)
    END DO ConstructAgeArray

    DO iage = 1, nage
      WRITE(*,*) "For age(iage)=", age(iage)
      ...
      !IteratePatches: DO P = 1, NPATCH2D
        ...
        !! We also need to catch the first iteration here- the rest of the code
        !! is configured to assume age_max=0 and i_max=0 when there are no
        !! cohorts with age greater than age(iage), so we can't just start with
        !! age_max=HUGE(age_max), i_max=HUGE(i_max)
        !IF ((POP%POP_grid(g)%patch(P)%age(1) >= age(iage)) .AND.&
            !((POP%POP_grid(g)%patch(P)%age(1) < age_max) .OR.&
            !(age_max == 0))) THEN
      ...
      IF (ANY(PatchAges <= age(iage))) THEN
        i_min = MAXLOC(PatchAges, 1, PatchAges <= age(iage))
        age_min = PatchAges(i_min)
      ELSE
        i_min = 0
        age_min = 0
      END IF
      ...
      IF (ANY(PatchAges >= age(iage))) THEN
        i_max = MINLOC(PatchAges, 1, PatchAges >= age(iage))
        age_max = PatchAges(i_max)
      ELSE
        i_max = 0
        age_max = 0
      END IF
       ...
       WRITE(*,*) "age_min:", age_min, "i_min:", i_min, "age_max:", age_max,&
       "i_max:", i_max
       ...
       if ((i_min.gt.0).and.(i_max.gt.0).and.(age_max.eq.age_min)) then
          ...
          IteratePatches_2: DO P = 1, NPATCH2D
            IF (POP%POP_grid(g)%patch(P)%age(1) == age_min) THEN
              ...
              cmass_age(iage) = cmass_age(iage) +&
                  POP%POP_grid(g)%patch(P)%layer(1)%biomass
          ...
          elseif  ((i_max.eq.0).and.(i_min.gt.0)) then
            ...
            i_min = MAXLOC(PatchAges, 1, PatchAges < age_max)
            age_min = PatchAges(age_min)
            !age_min = MAXVAL(pop%pop_grid(g)%patch(:)%age(1), &
          ...
          elseif (((age_min_growth.LE.2).OR.(i_min_growth.eq.0)).and. &
            ...
            i_max_growth = MINLOC(PatchAges, 1, PatchAges > age_min_growth)
            age_max_growth = PatchAges(i_max_growth)
          ...
          elseif  ((i_max_growth.eq.0).and.(i_min_growth.gt.0)) then
            ...
            i_min_growth = MAXLOC(PatchAges, 1, PatchAges < age_max_growth)
            age_min_growth = PatchAges(i_min_growth)
          ...
          if (i_min.ne.0.and.age_min.ne.0) then
            ...
            WRITE(*,*) "Biomass_min:", biomass_min
            WRITE(*,*) "sapwood_min:", sapwood_min
            WRITE(*,*) "sapwood_area_min:", sapwood_area_min
            WRITE(*,*) "stress_mort_min:", stress_mort_min
            WRITE(*,*) "crowd_mort_min:", crowd_mort_min
          ...
          WRITE(*,*) "Before averaging"
          WRITE(*,*) "Biomass_max:", biomass_max
          WRITE(*,*) "sapwood_max:", sapwood_max
          WRITE(*,*) "sapwood_area_max:", sapwood_area_max
          WRITE(*,*) "stress_mort_max:", stress_mort_max
          WRITE(*,*) "crowd_mort_max:", crowd_mort_max
          ...
          if (i_min_growth.ne.0.and.age_min_growth.ne.0) then
            ...
            IteratePatches_8: DO P = 1, NPATCH2D
              IF (PatchAges(P) == age_min_growth) THEN
          ...
          IteratePatches_9: DO P = 1, NPATCH2D
            IF (PatchAges(P) == age_max_growth) THEN
          ...
          WRITE(*,*) "age_min_growth:", age_min_growth, "i_min_growth:", i_min_growth,&
                "age_max_growth:", age_max_growth, "i_max_growth:", i_max_growth
       ...
       WRITE(*,*) "Cmass_sum:", POP%pop_grid(g)%cmass_sum
       WRITE(*,*) "Csapwood_sum:", POP%pop_grid(g)%csapwood_sum
       WRITE(*,*) "Sapwood_area:", POP%pop_grid(g)%sapwood_area
!!$if (g==2) then
...
!*******************************************************************************
  ...
  SUBROUTINE alloc_POP(POP, arraysize)
    ...
    INTEGER :: LS, P, L ! Iterate over landscapes, patches, layers
    ...
    AllocateLandscapes: DO LS = 1, arraysize
      ...
      AllocatePatches: DO P = 1, NPATCH2D
        ...
        ALLOCATE(POP%POP_grid(LS)%patch(P)%layer(NLayer))
        AllocateLayers: DO L = 1, NLAYER
          ALLOCATE(POP%POP_grid(LS)%patch(P)%layer(L)%Cohort(NCOHORT_MAX))
        END DO AllocateLayers
  ...
  SUBROUTINE update_POP_parameters(disturbance_interval, disturbance_intensity)
    ...
    NAMELIST /popnml/ fulton_alpha, densindiv_max, densindiv_min, kbiometric,&
                      ...
                      patch_reps2, height_bins, bin_power, nlayer,&
