MODULE crop_module
  use cable_def_types_mod, only: &
      climate_type, soil_snow_type, soil_parameter_type, &
  ...
  public :: C_allocation_crops
  public :: heat_units
  public :: SLA_development
  public :: irrigation
...
contains
  ...
  subroutine planting(i, doy, climate, ssnow, soil, crop)
    !! Determines planting/sowing date.
    ...
    ! cumulative soil layer depths (bottom) (m)
    real, dimension(size(soil%zse)) :: cumzse
    ...
    rSM_avail = available_soil_moist( &
        real(ssnow%wb(i,crop%sl(i))), &
        soil%sfc(i), &
        soil%swilt(i) &
        )
    ...
    if (i == 1) then ! winter wheat
      if ( &
          maxval( &
              climate%dtemp_31(i,(31-nrdays_before+1):31) &
              )<=crop%Tplant(i) .and. & ! temperature
          minval( &
              climate%dtemp_31(i,(31-nrdays_before+1):31) &
              )>crop%Tbase(i) .and. & ! temperature
    ...
    write(65,*) 'doy:', doy
    write(65,*) '  climate%dtemp_31(i,(31-nrdays_before+1):31):', &
        climate%dtemp_31(i,(31-nrdays_before+1):31)
  ...
  subroutine germination(i, doy, climate, ssnow, soil, crop)
    ...
    if (crop%state_nrdays(i,planted) == nrdays_after .and. &
        doy < crop%sowing_doymax(i)) then
        ! TODO: ensure no problems when switching years!
      if (i == 1) then ! winter wheat
        if (maxval( &
                climate%dtemp_31(i,(31-nrdays_after+1):31))>18.0 .or. &
            minval( &
                climate%dtemp_31(i,(31-nrdays_after+1):31))<crop%Tbase(i)) then
          planting_ok = .FALSE.
        endif
      else
        if ( &
    ...
    ! (based on temp in soil layer crop%sl)
    fPHUger = heat_units( &
        climate%dtempsoil(i,crop%sl(i)), &
        crop%Tbase(i),crop%Tmax(i) &
        )/crop%PHU_germination(i)

    ! calculate soil water requirements for germination
    fwsger = available_soil_moist( &
        real(ssnow%wb(i,crop%sl(i))), &
        fcrit_germination*soil%sfc(i), &
        soil%swilt(i) &
        )
    ...
    if (crop%state_nrdays(i,planted)>maxdays_ger .or. .not. planting_ok) then
      ! germination assumed to have failed
      crop%state(i) = baresoil ! bare soil again, no plant growth
      crop%sowing_doy = 0
      crop%fgermination(i) = 0.0
    else if ( &
        crop%fgermination(i)>=1.0_dp .and. &
        crop%state_nrdays(i,planted)<=maxdays_ger .and. &
        planting_ok) then
    ...
    write(65,*) 'doy:', doy
    write(65,*) '  climate%dtempsoil(:,crop%sl(1)):', &
        climate%dtempsoil(:,crop%sl(1))
  ...
  subroutine emergence( &
    ...
    casapool%dcplantdt(i,:) = casaflux%fracCalloc(i,:)*crop%Cseed(i)*fPHU_day &
        /crop%fPHU_emergence(i)
    write (60,*) '  casapool%dcplantdt(i,:):', casapool%dcplantdt(i,:)
    casapool%dcplantdt(i,:) = casapool%dcplantdt(i,:) + &
        (max(casaflux%Cnpp(i),0.0_dp)*casaflux%fracCalloc(i,:))
    write (60,*) '  casapool%dcplantdt(i,:):', casapool%dcplantdt(i,:)
    ...
    crop%Cstem_mobile(i) = crop%Cstem_mobile(i) + &
        crop%fCstem_mobile(i)*casapool%dcplantdt(i,wood)
    ...
    veg%hc(i) = vegheight_Cstem_Cleaf( &
        casapool%Cplant(i,wood), &
  ...
  subroutine vernalisation(i, climate, crop)
    ...
    crop%fVU(i) = min((crop%VU(i)**5.0_dp)/(22.5_dp**5.0_dp + &
        crop%VU(i)**5.0_dp),1.0_dp)

    write(60,*) ' crop%fVU(i):', crop%fVU(i)

    if (crop%fPHU(i) >= 0.45_dp) then
      crop%fPHU(i) = crop%fPHU(i) * crop%fVU(i)
      ! alternative: increase fPHU_maturity.
  ...
  subroutine growth(i, SLA_C, veg, casaflux, casapool, casamet, crop)
    !! Plant growth in vegetative and reproductive phase.
    ...
    write(60,*) '  casaflux%fracCalloc(i,product):', &
    ...
    if ( &
        casapool%dcplantdt(i,leaf) > 0.0_dp .or. &
        casapool%dcplantdt(i,wood) > 0.0_dp) then
      ! update vegetation height
      veg%hc(i) = vegheight_Cstem_Cleaf( &
          casapool%Cplant(i,wood), &
          casapool%Cplant(i,leaf) &
  ...
  subroutine C_allocation_crops(i, casaflux, crop)
    !! C allocation to plant pools (crops only).
    ...
    casaflux%fracCalloc(i,froot) = Calloc_roots( &
        crop%fPHU(i), &
        crop%fCalloc_root_init(i), &
        crop%Calloc_root_end(i) &
        )
    casaflux%fracCalloc(i,leaf) = Calloc_leaves( &
        crop%fPHU(i), &
        crop%Calloc_leaf_bpt(i), &
        crop%Calloc_leaf_end(i), &
        crop%fCalloc_leaf_init(i), &
        crop%fCalloc_leaf_bpt(i) &
        )
    casaflux%fracCalloc(i,product) = Calloc_products( &
        crop%fPHU(i), &
        fPHU_flowering, &
        crop%Calloc_prod_max(i) &
        )
    casaflux%fracCalloc(i,wood) = 1.0_dp - &
        (casaflux%fracCalloc(i,froot) + casaflux%fracCalloc(i,leaf) + &
        casaflux%fracCalloc(i,product))
  ...
  subroutine senescence(i, fPHU_day, casamet, casapool, casaflux, crop)
    !! Plant senescence and effects on C pools and LAI + C remobilisation.
    !! Models sequential senescence (constant leaf senescence due to e.g.
    !! ageing) and reproductive senescence (occurs after flowering).
    ...
    ! --> at the moment stems and roots do not experience mass loss due to
    ! respiration at senescence, i.e. any respiration C is coming from GPP!
    ...
    if (crop%fPHU(i) > crop%Calloc_leaf_end(i)) then
      rsenesc = ((crop%fPHU(i) - crop%Calloc_leaf_end(i)) / &
          ...
          ((max( &
              crop%fPHU(i) - fPHU_day, &
              crop%Calloc_leaf_end(i) &
              ) - crop%Calloc_leaf_end(i))/sen_period)**crop%drsen(i)
    else
       rsenesc = 0.0_dp
    endif
    ...
    casaflux%Cnpp(i) = casaflux%Cgpp(i) - &
        sum(casaflux%Crmplant(i,:)) - casaflux%Crgplant(i)
    write(60,*) '  casaflux%Crmplant2(i,:):', casaflux%Crmplant(i,:)

    ! Calculate C loss in leaves due to senescence
    ! Part of this C loss is respired, the other part is mobilised to 
    ! the product
    ...
    ! update NPP: usually, C for Rm is taken from GPP, but in senescence it
    ! is taken directly from the C pools in the leaves/stems. Thus updating
    ! NPP to conserve C mass balance.
    casaflux%Cnpp(i) = casaflux%Cnpp(i) + &
        (sum(casaflux%Crmplant(i,(/leaf,wood/))) - C_loss_resp)

    write(60,*) '  sum(casaflux%Crmplant(i,(/leaf,wood/))):', &
        sum(casaflux%Crmplant(i,(/leaf,wood/)))
  ...
  subroutine remobilisation(i, casapool, crop, fPHU_day, Closs_leaf_mobile)
    ...
    if (crop%fPHU(i) > crop%Calloc_prod_max(i) .and. crop%fPHU(i) < &
      ...
      rCloss_stem = ((crop%fPHU(i) - crop%Calloc_prod_max(i)) / &
          ...
          crop%Calloc_prod_max(i))/remob_period)**0.5_dp
    else
       rCloss_stem = 0.0_dp
    endif
    ! avoid fsenesc exceeding 1.0
    if (rCloss_stem + crop%fmobilise(i) > 1.0_dp) then
        rCloss_stem = rCloss_stem - ((rCloss_stem + crop%fmobilise(i)) - &
            1.0_dp)
    endif
  ...
  subroutine irrigation(i, ssnow, soil, canopy)
    ...
    ! mean soil moisture (m3/m3) in soil columns used for calc. irrg. demand
    real :: wb_mean
    ...
    wb_avail = max( &
        0.0, &
        min(1.0, (wb_mean - soil%swilt(i))/(soil%sfc(i)-soil%swilt(i))) &
        )

    ! determine total irrigation demand based on available soil moisture in
    ! layers 1:sl and a irrigation loss factor (0-1)
    irrig_demand = 0.0
    if (wb_avail < irrig_trigger) then ! trigger irrigation
       do s=1,sl
          irrig_demand = irrig_demand + &
              max(0.0, (soil%sfc(i) - ssnow%wb(i,s))*soil%zse(s)*1000.0 + &
              Firrig_loss*(soil%ssat(i) - soil%sfc(i))*soil%zse(s)*1000.0)
          ! equivalent to that:
          !irrig_demand = irrig_demand + &
          !    max(0.0, ((Firrig_loss * (soil%ssat(i) * soil%zse(s)) + &
          !    max(0.0,(1.0 - Firrig_loss)) * (soil%sfc(i) * soil%zse(s))) - &
          !    ssnow%wb(i,s) * soil%zse(s)) * 1000.0)
       !irrig_demand = max( &
       !    0.0, &
       !    ((soil%sfc(i) - wb_mean)*cumzse(sl)*1000.0)/irrig_eff, &
       !    )
       !irrig_demand = irrig_demand + max( &
       !    0.0, &
       !    ((soil%sfc(i)*soil%zse(s)) - (ssnow%wb(i,s)*soil%zse(s)))*1000.0)
       end do
    endif
    ...
    canopy%irrig_surface(i) = 0.0 ! mm/d
    ! IMPORTANT: ensure agreement among tile indices
  ...
  subroutine harvest(i, doy, casapool, casamet, veg, crop)
    !! Removal of harvested products and redistribution of remaining biomass
    ...
    crop%harvest_index(i) = casapool%Cplant(i,product) / &
        sum(casapool%Cplant(i,(/leaf,wood,product/))) ! diagnostic
    ...
    Cstemleaf_removed = sum( &
        (1.0_dp - crop%Cplant_remove)*casapool%Cplant(i,leaf:wood))
    ! add to dt variable first?
    casapool%Clitter(i,str) = casapool%Clitter(i,str) + Cstemleaf_removed
  ...
  function heat_units(dtemp, basetemp, maxtemp) result (HU)
    ...
    HU = min( &
        max(0.0, dtemp - max(0.0, basetemp)), &
        maxtemp &
        )
  ...
  function available_soil_moist(theta, theta_fc, theta_wilt) result (favail)
    ...
    !! All inputs in m3/m3, result is between 0 and 1.
  ...
  function Calloc_leaves( &
    ...
    if (fPHU <= fPHU_bpt) then
       fCalloc = fCalloc_init - &
           1.0_dp/fPHU_bpt*(fCalloc_init - fCalloc_bpt)*fPHU
    else if (fPHU > fPHU_bpt .and. fPHU < Calloc_end) then
       fCalloc = fCalloc_bpt - &
           fCalloc_bpt/(Calloc_end - fPHU_bpt)*(fPHU - fPHU_bpt)
    else
       fCalloc = 0.0_dp
    endif
